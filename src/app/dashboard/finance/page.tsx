...
import { listEntities, listFilingsForEntity, getFilingKpis } from "@/lib/finance/queries";
import { formatAmount, formatPeriod } from "@/lib/finance/format";
import { NewFinanceEntityForm } from "@/components/new-finance-entity-form";

export default async function FinanceEntitiesPage() {
  const org = await requireCurrentOrg();
  const entities = await listEntities(org.organizationId);
  ...
  return (
    <div>
      ...
      <p className="mt-1 text-sm text-[var(--sub)]">
        법인별 월간 재무제표(资产负债表·利润表·现金流量表)를 업로드해 매출·순이익·현금흐름 추이를 관리합니다.
      </p>

      {canEdit(org.role) && (
        <div className="mt-4">
          <NewFinanceEntityForm />
        </div>
      )}
      ...
      {cards.length === 0 ? (
        ...
        <ul className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2">
          {cards.map(({ entity, latest, kpis, filingCount }) => (
            <li key={entity.id}>
              <Link
                href={`/dashboard/finance/${entity.id}`}
