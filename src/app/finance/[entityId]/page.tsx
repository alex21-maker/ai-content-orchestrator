...
import { loadEntityPublic, listFilingsForEntity, getFilingKpis, type FinanceKpis } from "@/lib/finance/queries";
...
export default async function PublicFinanceEntityDetailPage({ params }: { params: Promise<{ entityId: string }> }) {
  const { entityId } = await params;

  const entity = await loadEntityPublic(entityId);
  if (!entity) notFound();
  ...
  return (
    <div>
      <Link href="/finance" className="text-xs text-[var(--sub)] hover:text-[var(--accent)]">
        ← 법인 목록
      </Link>
      ...
      {latest && latestKpis ? (
        ...
        <p className="mt-6 text-sm text-[var(--sub)]">아직 업로드된 재무제표가 없습니다.</p>
      )}
        ...
        <div className="mt-6">
          ...
          <ul className="mt-2 flex flex-col gap-2">
            {filingsDesc.map((filing) => (
              <li key={filing.id}>
                <Link
                  href={`/finance/${entity.id}/filings/${filing.id}`}
