...
리스트는 **프로젝트별로 묶여서** 표시됩니다 — 입력한 "회의/프로젝트 이름"이 메인
`ai-content-orchestrator` 앱(`proj.lablab.cloud`)의 캠페인 이름과 일치하면(대소문자 무시) 그
캠페인 아래로 묶이고, 일치하는 캠페인이 없으면 **기타** 그룹으로 묶입니다. 이 매칭은 이름만으로
이루어지며(두 앱 사이에 실제 FK 관계는 없음), 캠페인 상세 페이지에도 같은 이름 매칭 방식으로
회의 타임라인이 표시됩니다.
...
| `DATABASE_URL` | 회의록 리스트 저장용 Postgres 연결 문자열. 메인 `ai-content-orchestrator` 앱과 같은 Supabase 프로젝트를 재사용합니다 — 이 앱 전용 `standalone_meeting_notes` 테이블(첫 요청 시 자동 생성)에 쓰고, 프로젝트별 그룹핑을 위해 메인 앱의 `campaigns` 테이블을 읽기 전용으로 조회합니다. 없어도 녹음/분석/Slack 전송은 정상 동작하고, 리스트·삭제·그룹핑 기능만 비활성화됩니다. |
...
**앱을 게시하지 않으면 정확히 7일 후 refresh token이 만료되고, 이후 모든 요청이
`invalid_grant` 오류로 실패합니다** (2026-09 초에 실제로 이 문제로 약 일주일간 회의록이 하나도
저장되지 않았습니다). 반드시 **Google 인증 플랫폼 → 대상(Audience) → 게시(Publish)** 까지
완료하세요. `drive.file` 범위는 Google의 재검토가 필요한 민감/제한 범위가 아니라서 게시 자체는
막힘없이 바로 됩니다 (브랜딩 정보 — 앱 이름, 지원 이메일 — 입력만 완료하면 게시 버튼이
활성화됩니다).

## 알려진 제한사항

- **요청 본문 크기**: Vercel 서버리스 함수는 요청 본문이 약 4.5MB로 제한됩니다. 긴 회의는 녹음이
  그 용량에 가까워질 때마다 자동으로 새 파트로 나뉘어 백그라운드에서 업로드/분석되며, 녹음
  자체는 "회의 종료"를 누르기 전까지 끊기지 않습니다. 각 파트는 "프로젝트명 (N)" 형태로 리스트에
  개별 항목으로 쌓입니다.
...
Google Drive 업로드 실패(예: refresh token 만료)나 회의록 목록 저장 실패가 발생하면, `SLACK_WEBHOOK_URL`이
설정되어 있는 경우 즉시 Slack으로 🚨 알림이 전송됩니다. 녹음하는 사람 화면에만 에러가 뜨고 아무도
못 보고 지나가서, 실제로 며칠~일주일씩 회의록이 조용히 유실됐던 적이 있어 추가한 안전장치입니다.
